			...
			for(data_size = scanner.nextInt(); data_size > 0; data_size--)
			{
				...
				for(int f = 0; f < largest_list.size() && largest_found == false; f++)
				{
					...
					for(int r = 0; r < rows; r++)
					{
						...
						{
							if(matrix[r][c]>largest_size2)//Check all 4 corners
							{
								//no overlapping rows or columns
								if(r+matrix[r][c]-1 > C.row() || r > C.row()+C.length()-1)
								{
									if(C.col()-C.length()+1 > c || c-matrix[r][c]+1 > C.col())
									{
										largest_size2 = matrix[r][c];
									}
								}
								//overlapping row, non-overlapping column
								else if(r+matrix[r][c]-1 < C.row() || r < C.row()+C.length()-1)
								{
									if(C.col()-C.length()+1 > c || c-matrix[r][c]+1 > C.col())
									{
										largest_size2 = matrix[r][c];
								...
								//overlapping column, non-overlapping row
								else if(r+matrix[r][c]-1 > C.row() || r > C.row()+C.length()-1)
								{
									if(C.col()-C.length()+1 < c || c-matrix[r][c]+1 < C.col())
									{
							...
							{//if 2nd largest is same as largest then we are done
								//System.out.println("Two equal sizes");
								largest_found = true;
							}
						}
					}
