	...
	public void Squares() throws Exception
	{
		try{
			...
			for(data_size = scanner.nextInt(); data_size > 0; data_size--)
			{
						...
						else if(index_info == 'F')//If index contains a free spot
						{
							...
							{	//Take minimum value of (index above, left, and diagonally left above) + 1
								...
								if(matrix[i][j]>1)
								{
									...
									if(matrix[i-1][j-1] >= matrix[i][j] && diag_check == false)//diagonal up left
									{
									...
									//Remove if less than above input by 2 or more since it will never be selected
									if(matrix[i][j]+2 < matrix[i][j-1] || matrix[i][j]+2 < matrix[i-1][j] || matrix [i][j]+2 < matrix[i-1][j-1])
									{
										squares_quantity[(matrix[i][j])]-=1;
										if(squares_quantity[(matrix[i][j])]==-1)
										{
											squares_quantity[(matrix[i][j])]=0;
										}
									}
								}
							}
				...
				squares_quantity[largest_size]=(int)Math.floor((double)squares_quantity[largest_size]/(double)largest_size);
				squares_quantity[largest_size]-=1; //
				for(int sq = largest_size; sq > 0 && largest_size2 == 0; sq--)
				{
					//System.out.println(sq + " " + squares_quantity[sq]);
					if(squares_quantity[sq] > sq && squares_quantity[sq]>0)
					{
						squares_quantity[sq]=(int)Math.floor((double)squares_quantity[sq]/(double)sq);
					}
